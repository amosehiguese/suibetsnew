...
class SettlementWorkerService {
  ...
  public async checkAndSettleBets() {
    ...
    try {
      ...
      await this.autoSettleGeneratedSports(pendingBets);
    } catch (error) {
      console.error('❌ SettlementWorker checkAndSettleBets error:', error);
    }
  }

  private static readonly GENERATED_EVENT_PREFIXES = [
    'wwe_', 'boxing_', 'f1_', 'motogp_', 'tennis_',
    'horse-racing_fb-', 'ufc_', 'ufc-',
    'cricket_', 'nba_', 'nhl_', 'mlb_', 'afl_',
    'handball_', 'rugby_', 'volleyball_',
  ];

  private isGeneratedEvent(eventId: string): boolean {
    return SettlementWorkerService.GENERATED_EVENT_PREFIXES.some(p => eventId.startsWith(p));
  }

  private async autoSettleGeneratedSports(pendingBets: UnsettledBet[]): Promise<void> {
    const now = new Date();
    const SETTLE_DELAY_MS = 3 * 3600 * 1000;

    const generatedSingleBets = pendingBets.filter(bet => {
      if (this.isParlayBet(bet)) return false;
      const extId = bet.externalEventId || '';
      return this.isGeneratedEvent(extId);
    });

    if (generatedSingleBets.length === 0) return;

    const { freeSportsService } = await import('./freeSportsService');
    const allCachedEvents = freeSportsService.getCachedEvents?.() || [];

    let settled = 0;
    for (const bet of generatedSingleBets) {
      if (this.isEventSettled(bet.externalEventId)) continue;

      const cachedEvent = allCachedEvents.find((e: any) => e.id === bet.externalEventId);
      if (!cachedEvent) {
        const eventDate = this.extractDateFromEventId(bet.externalEventId);
        if (!eventDate) continue;
        const eventEnd = new Date(eventDate.getTime() + SETTLE_DELAY_MS);
        if (now < eventEnd) continue;

        console.log(`🎭 Auto-settling generated event (no cache): ${bet.externalEventId}`);
        const prediction = (bet.prediction || '').trim();
        const homeTeam = (bet.homeTeam || '').trim();
        const isHome = prediction.toLowerCase() === homeTeam.toLowerCase();
        const winner = isHome ? 'home' : 'away';

        const fakeMatch: FinishedMatch = {
          eventId: bet.externalEventId,
        ...
        await this.settleBetsForMatch(fakeMatch, [bet]);
        await this.persistSettledEvent(fakeMatch, 1);
        settled++;
        continue;
      }

      const eventStart = new Date(cachedEvent.startTime);
      const eventEnd = new Date(eventStart.getTime() + SETTLE_DELAY_MS);
      if (now < eventEnd) continue;

      let winnerName = '';
      if (cachedEvent.markets && cachedEvent.markets.length > 0) {
        const outcomes = cachedEvent.markets[0].outcomes || [];
        if (outcomes.length >= 2) {
          const sorted = [...outcomes].sort((a: any, b: any) => (a.odds || 99) - (b.odds || 99));
          winnerName = sorted[0].name;
        }
      }

      if (!winnerName) {
        winnerName = cachedEvent.homeTeam || bet.homeTeam || 'Home';
      }

      const isHomeWinner = winnerName.toLowerCase() === (cachedEvent.homeTeam || '').toLowerCase();
      const fakeMatch: FinishedMatch = {
        eventId: bet.externalEventId,
        homeTeam: cachedEvent.homeTeam || bet.homeTeam || 'Home',
        awayTeam: cachedEvent.awayTeam || bet.awayTeam || 'Away',
      ...
      console.log(`🎭 Auto-settling generated event: ${bet.externalEventId} → Winner: ${winnerName}`);
      const allBetsForEvent = pendingBets.filter(b => b.externalEventId === bet.externalEventId);
      await this.settleBetsForMatch(fakeMatch, allBetsForEvent);
      await this.persistSettledEvent(fakeMatch, allBetsForEvent.length);
      settled++;
    }

    if (settled > 0) {
      console.log(`🎭 Auto-settled ${settled} generated sport events`);
    }

    const generatedParlayBets = pendingBets.filter(bet => {
      if (!this.isParlayBet(bet)) return false;
      const pred = bet.prediction || '';
      return SettlementWorkerService.GENERATED_EVENT_PREFIXES.some(p => pred.includes(p));
    });

    if (generatedParlayBets.length > 0) {
      const generatedFinished: FinishedMatch[] = [];
      for (const ev of allCachedEvents) {
        const evStart = new Date(ev.startTime);
        const evEnd = new Date(evStart.getTime() + SETTLE_DELAY_MS);
        if (now >= evEnd && this.isGeneratedEvent(ev.id)) {
          let winnerName = ev.homeTeam;
          if (ev.markets && ev.markets.length > 0) {
            const outcomes = ev.markets[0].outcomes || [];
            if (outcomes.length >= 2) {
              const sorted = [...outcomes].sort((a: any, b: any) => (a.odds || 99) - (b.odds || 99));
              winnerName = sorted[0].name;
            }
          }
          const isHome = winnerName.toLowerCase() === (ev.homeTeam || '').toLowerCase();
          generatedFinished.push({
            eventId: ev.id,
            homeTeam: ev.homeTeam,
            awayTeam: ev.awayTeam,
            homeScore: isHome ? 1 : 0,
            awayScore: isHome ? 0 : 1,
            winner: isHome ? 'home' : 'away',
            status: 'finished'
          });
        }
      }
      if (generatedFinished.length > 0) {
        await this.settleParlayBets(generatedParlayBets, generatedFinished);
      }
    }
  }

  private extractDateFromEventId(eventId: string): Date | null {
    const dateMatch = eventId.match(/(\d{4}-\d{2}-\d{2})/);
    if (dateMatch) {
      const d = new Date(dateMatch[1] + 'T01:00:00Z');
