...
- **Generated Sports Schedules**: F1 (sportId=11) has full 2026 calendar with 22-driver grid (Cadillac, Audi, Racing Bulls teams), shows next 5 races. UFC/MMA (sportId=7) has 18 generated fight cards through June 2026 (UFC 327, 328, Freedom Fights 250). These supplement the free API which only returns today/tomorrow's events.
...
- **WWE Entertainment**: Generated events from real WWE PLE schedule (WrestleMania, Backlash, SummerSlam, Money in the Bank, etc.) plus dynamically generated weekly Raw (Monday 01:00 UTC) and SmackDown (Friday 01:00 UTC) shows for the next 8 weeks. Sport ID 20. Odds modeled on sportsbook-style wrestling odds. Weekly shows use `generateWeeklyWWEShows()` with rotating matchups and venues, computed in UTC to avoid timezone drift. Events include 19 PLEs + ~32 weekly shows.
